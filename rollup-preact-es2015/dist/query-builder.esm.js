...
function Queries(props) {
  const queries = props.queries.map(query => h(
    "span",
...
function FilterInput(props) {
  function renderInput(type) {
    switch (type) {
      ...
      case "ENUM":
        const enumOptions = props.enum.map(item => h(
          "option",
          { key: item, value: item },
          item
        ));
...
class Filter extends Component {
  ...
  render() {
    const organizationOptions = this.props.entities.map(entity => h(
      "option",
      { key: entity.id, value: entity.id },
      entity.label
    ));

    const attributeOptions = this.state.attributes[this.state.selectedOrganization].map(attribute => h(
      "option",
      { key: attribute, value: attribute },
      attribute
    ));

    const operatorOptions = this.state.operators.map(operator => h(
      "option",
      { key: operator.id, value: operator.id },
      operator.label
    ));

    const combinatorOptions = this.props.combinators.map(combinator => h(
      "option",
      { key: combinator, value: combinator },
      combinator
    ));

    const validInputs = this.state.selectedOrganization && this.state.selectedAttribute && this.state.selectedOperator && this.state.selectedInput;
...
class QueryBuilder extends Component {
  ...
  componentDidMount() {
    const response = fetch(CONSTANTS.ENDPOINT).then(response => response.json()).then(json => this.setState({ data: json }));
  }

  render() {
    const content = this.state.data ? h(Panel, {
